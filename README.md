...
주간 화면의 **[한글 문서로 저장]** 은 그 주 기록을 주간업무 일지 양식(`hwpx`)의
"지난주 실적" 칸에 채워 넣는다. 한글을 띄우지 않고 파일만 만든다.

누르면 미리보기가 뜬다. 각 줄의 단계(`1. 제목` / `ㅇ 항목` / `- 세부`)를 앱이 추측해
표시하고, 클릭하면 단계가 바뀐다. 일지는 평평하게 쓰이니 제목인지 아닌지는 추측일 수밖에
없고, 보고서로 나가는 문서라 조용히 적용하지 않고 한 번 보여준다.

양식은 exe 안에 들어 있다. `%APPDATA%\DailyWorkAlter\template.hwpx` 를 두면 그쪽이
우선이라 재빌드 없이 바꿀 수 있다. 오른쪽 "이번주 계획" 칸은 비워서 내보낸다 — 앱은
한 일만 기록하고 할 일은 받지 않는다.
...
- **퇴근 30분 전부터 퇴근시간까지** 뜨고, 빈 채로 닫으면 `repeatMinutes`마다 다시 뜬다
- 그날 첫 알림은 이미 써둔 내용이 있어도 뜬다 — 점심에 한 줄 적어둔 게 퇴근 전 정리를
  건너뛸 이유는 아니다. 다만 한 번 뜬 뒤 기록에 내용이 있으면 더 안 뜬다
